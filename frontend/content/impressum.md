# Impressum und Lizenzhinweise

## Projektverantwortung

**Stadt und Gemeinschaft Wien — Datenbank zu mittelalterlichen Wiener Rechtsgeschäften**
Prosopographische Datenbank mittelalterlicher Wiener Rechtsgeschäfte (ca. 1177–1524)

**Projektleitung:**
Mag. Korbinian Grünwald, Universität Wien

**Forschungsprojekte:**
- „Im Netz der Stadt" (Dissertation, Universität Wien)
- „Stadt und Gemeinschaft" (Universität Wien)

## Technische Umsetzung

**Modernisierung und Digitale Edition:**
Digital Humanities Craft OG

**Technische Basis:**
- TEI-XML Quelldaten mit RelaxNG-Schema
- Python/lxml Transformationspipeline
- Statische Website (GitHub Pages) mit Jinja2-Templates

## Lizenz

Die Inhalte dieser digitalen Edition stehen unter der **Creative Commons Attribution 4.0 International License (CC BY 4.0)**.

Sie dürfen:
- **Teilen** — das Material in jedwedem Format oder Medium vervielfältigen und weiterverbreiten
- **Bearbeiten** — das Material remixen, verändern und darauf aufbauen

Unter folgenden Bedingungen:
- **Namensnennung** — Sie müssen angemessene Urheber- und Rechteangaben machen

Volltext der Lizenz: [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)

## Datenquellen

**Faksimile-Bilder:** [Monasterium.net](https://www.monasterium.net/) — Virtuelles Archiv mittelalterlicher Urkunden Europas

**Editionsgrundlage:** Quellen zur Geschichte der Stadt Wien (QGW), Stadtbücher, Satzbuch CD

## Zitierweise

Bei Verwendung der digitalen Edition bitte wie folgt zitieren:

> Grünwald, Korbinian: Stadt und Gemeinschaft Wien — Datenbank zu mittelalterlichen Wiener Rechtsgeschäften. Universität Wien. URL: [Permalink]. Zugriff: [Datum].

Einzeldokumente können über den Zitierhelfer (Anführungszeichen-Button auf jeder Dokumentseite) in Chicago- und BibTeX-Format exportiert werden.

## Kontakt

Für Fragen zur Edition, den Daten oder zum Forschungsprojekt wenden Sie sich bitte an die Projektleitung an der Universität Wien.
