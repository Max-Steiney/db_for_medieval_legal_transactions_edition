...
**Anforderung.** Die Anwendung unterscheidet zwei Sichtbarkeits-Schichten. Build-zeit greift eine Audience-Achse (`--audience public|private`, Begriffsbildung analog zu GitHub-Repos), die ganze Sektionen und Datenpunkte vor dem Rendern aus dem Output filtert. Client-zeit greift ein Dev-Mode-Schalter (`?dev=1` an der URL), der Elemente mit der CSS-Klasse `.dev-only` im selben Build sichtbar macht. Die zwei Schichten sind orthogonal und addieren sich.
