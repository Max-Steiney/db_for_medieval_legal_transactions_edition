...
Orthogonal zum Stufenmodell führt der Build eine zweite Achse, die unterscheidet, *für wen* der Output gedacht ist. CLI-Auslöser ist `--audience oeffentlich|intern` (Default `oeffentlich`). Die Mechanik lebt in `frontend/audiences.py` analog zu `frontend/stages.py` und setzt `FRONTEND_AUDIENCE` als Env-Var. Intern hängt das Suffix `-intern` an das Stage-Output-Verzeichnis (Stufe 1 plus intern landet in `docs-intern/`), Öffentlich schreibt unverändert in das Stufen-Verzeichnis.
