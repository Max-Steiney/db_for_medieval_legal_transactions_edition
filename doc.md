...
- [x] **Öffentlicher Build zeigt ausschließlich die freigegebenen Korpora (QGW II/1, StB Bd. 1), interner Build alle fünf.** (Herkunft: Stakeholder-Protokoll 18.05.2026, Prio 1 Punkt 1)
   ...
   * Offen: Drei Folgepunkte, getrennt zu behandeln.
       ...
       2. **`data-released-max="1460"` im body.** Befund dieser Session: das Attribut wird in `core.js` nach `window.RELEASED_PERIOD` geschrieben, aber von keinem Konsumenten gelesen; die echten Zeit-Schieberegler nehmen ihre Grenzen datengetrieben aus `min_year`/`max_year`. Die substanzielle Year-Range-Frage ist mit dem `docs_aggregate`-Fix bereits erledigt (Daten enden öffentlich bei 1414). Bleibt ein sachlich falsches, aber ungenutztes DOM-Attribut. Entscheidung offen: **entfernen** (toter `window.RELEASED_PERIOD`-Block plus body-Attribute, empfohlen) versus **sicht-abhängig machen**.
